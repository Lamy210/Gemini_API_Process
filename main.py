...
#answer"""
    ...
    def api_request_with_retry(self, row_num: int, problem: str, code: str) -> Tuple[str, str, int, str]:
        """APIリクエストを実行し、エラー時は再試行"""
        attempt = 0
        prompt = self.get_prompt_template(problem, code)
        base_delay = 2
        ...
        while True:
            try:
                ...
                if "429" in error_str:
                    ...
                    # 待機時間の計算を改善
                    delay = base_delay * (1 + random.random())  # base_delay〜base_delay*2 の範囲
    ...
    def process_excel(self, input_file: str, output_file: str) -> None:
    ...
    def process_row(self, row: pd.Series, row_num: int) -> Optional[Dict]:
        ...
        try:
            ...
            prompt, response, attempts, used_key = self.api_request_with_retry(row_num, problem, code)
    ...
    def get_prompt_template(self, problem: str, code: str) -> str:
        """Return the formatted prompt template"""
        return self.prompt_template.format(problem=problem, code=code)
...
def main():
    ...
    try:
        ...
        validator.process_excel('/app/input.csv', '/app/output.csv')
